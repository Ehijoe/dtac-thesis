...
\begin{titlepage}
    \begin{center}
        ...
        \includegraphics[width=4cm]{figures/UI_logo.png}
...
To address this vulnerability, this study engineers a Dynamic Traffic-Aware Cache (DTAC) architecture built in memory-safe Rust. The system introduces a Heuristic Proxy that evaluates passive HTTP anomalies, assigning a real-time probabilistic AI-score to incoming traffic in O(1) time complexity. This continuous score dynamically modifies the cache’s admission and randomized eviction logic, penalizing automated scans using a metadata-only Ghost Cache. Finally, the prototype is rigorously benchmarked against a standard LRU baseline using synthesized workloads of mixed human and AI traffic.
